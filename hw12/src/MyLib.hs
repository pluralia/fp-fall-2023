...
type LoggingStateWithErrorInIO stateType errorType resType =
  LoggerT (ExceptT errorType (StateT stateType IO)) resType


-- Теперь LoggerT снаружи, чтобы иметь возможность логировать ошибки, а затем ExceptT, чтобы обрабатывать ошибки, и, наконец, StateT и IO для работы со состоянием и выполнения IO-вычислений.
