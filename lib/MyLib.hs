...
-- -- Тип ошибки может быть расширен для предоставления более детальной информации
-- data YourError = FileNotFoundError | ParseError | UserNotFoundError UserId | UnknownCommandError String | StopCommandReceived

-- -- Функция для чтения и парсинга пользовательской базы
-- parseUserDB :: String -> Maybe UserDB
-- parseUserDB input = Map.fromList . catMaybes <$> traverse parseUserLine (lines input)
--   where
--     parseUserLine :: String -> Maybe (UserId, AccessRights)
--     parseUserLine line = case splitOn ':' line of
--       [userIdStr, rightsStr] -> 
--         case (readMaybe userIdStr, readMaybe rightsStr) of
--           (Just userId, Just rights) -> Just (userId, rights)
--           _ -> Nothing
--       _ -> Nothing

-- -- Функция для логирования, вспомогательная, использует интерфейс LoggerT
-- logInfo :: String -> LoggingStateWithErrorInIO YourState YourError ()
-- logInfo msg = lift . lift . LoggerT $ return (Logged [(Info, msg)] ())

-- -- Функция для прерывания с ошибкой
-- throwErrorLogging :: YourError -> LoggingStateWithErrorInIO YourState YourError a
-- throwErrorLogging err = do
--   logInfo $ "Error occurred: " ++ show err
--   throwError err

-- -- Загружаем базу пользователей и логируем это событие
-- loadUserDB :: FilePath -> LoggingStateWithErrorInIO YourState YourError ()
-- loadUserDB path = do
--   users <- liftIO (readFile path) catchError (const $ throwErrorLogging FileNotFoundError)
--   -- Предполагается функция parseUserDB, которая парсит файл и возвращает Map UserId AccessRights
--   case parseUserDB users of
--     Just db -> do
--       logInfo $ "Loaded user database from " ++ path
--       Control.Monad.State.put (YourState db)
--     Nothing -> throwErrorLogging ParseError

-- -- Вспомогательная функция для сохранения логов
-- writeToLogFile :: FilePath -> [String] -> IO ()
-- writeToLogFile path logEntries = writeFile path (unlines logEntries)

-- -- Функция для парсинга команды
-- parseCommand :: String -> Either YourError (Maybe (UserId, AccessRights))
-- parseCommand command = 
--   case words command of
--     ["GIVE", "ACCESS", userIdStr, accessRightsStr] -> 
--       case (readMaybe userIdStr, readMaybe accessRightsStr) of
--         (Just userId, Just accessRights) -> Right (Just (userId, accessRights))
--         _ -> Left ParseError
--     ["STOP"] -> Right Nothing
--     _ -> Left $ UnknownCommandError command

-- -- Функция для сохранения базы данных пользователей в строку
-- saveUserDB :: UserDB -> String
-- saveUserDB db = unlines $ map (\(uid, rights) -> show uid ++ ":" ++ show rights) $ Map.toList db

-- -- Обработчик команд
-- handleCommand :: String -> LoggingStateWithErrorInIO YourState YourError ()
-- handleCommand "STOP" = do
--   logInfo "Received STOP command"
--   throwErrorLogging StopCommandReceived
-- handleCommand command = case parseCommand command of
--   Left err -> throwErrorLogging err
--   Right Nothing -> throwErrorLogging $ UnknownCommandError command
--   Right (Just (userId, rights)) -> do
--     modify (\(YourState db) -> YourState $ Map.insert userId rights db)
--     logInfo $ "Access rights " ++ show rights ++ " assigned to user " ++ show userId

-- -- Считываем и обрабатываем команды
-- processCommands :: LoggingStateWithErrorInIO YourState YourError ()
-- processCommands = liftIO $ do
...
--   loadUserDB pathToInputDB

--   -- Обрабатываем команды
--   processCommands catchError handleErrors

--   -- Сохраняем обновленные логи и базу данных
--   YourState finalDB <- Control.Monad.State.get
--   (logs, ()) <- lift . lift $ runLoggerT (return ())
--   liftIO $ writeToLogFile pathToLog logs
--   -- Сохраняем базу данных пользователей
--   liftIO $ writeFile pathToOutDB (saveUserDB finalDB)

--   where
--     handleErrors StopCommandReceived = do
--       logInfo "Stopping and saving logs and database."
--       YourState finalDB <- Control.Monad.State.get
--       (logs, ()) <- lift . lift $ runLoggerT (return ())
--       liftIO $ do
--         writeToLogFile pathToLog logs
--         writeFile pathToOutDB (saveUserDB finalDB)
--     handleErrors err = do
