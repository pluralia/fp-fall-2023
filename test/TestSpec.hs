...
spec = do
  ...
  describe "Functor instance for Pair" $ do
    it "Functor applies function to list" $ do
      fmap length ((/ 2) :: Int -> Int) [2, 4, 6, 8, 10] `shouldBe` [1, 2, 3, 4, 5]
  ...
  describe "biFunctor" $ do
    it "biFunctor applies function to Either'" $ do
      let input  = Pair ((42 :: Int) "Приключение на 5 минут, ")
      let output = Pair ((84 :: Int) "Приключение на 5 минут, зашли и вышли")
      bimap ((* 2) :: Int -> Int) ((++ "зашли и вышли") :: String -> String) input `shouldBe` output
