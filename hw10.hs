...
-- 2. Запись в файл с буфером и без (1 балл)
...
-- Прочитайте коротенькую статью и посмотрите на hSetBuffering и hPutStrLn, затем ответьте на вопросы
-- https://mmhaskell.com/blog/2022/5/16/bufferingplease-wait

-- 1) В чем суть записи с буфером?
-- 2) Зачем нужна hFlush?
-- 3) Почему вы получили такое время работы при запуске writeToFile и writeToFileWithBuffer?
-- 4) Какой способ самый быстрый и почему: без или с буфером, если с буфером, то какого типа?
